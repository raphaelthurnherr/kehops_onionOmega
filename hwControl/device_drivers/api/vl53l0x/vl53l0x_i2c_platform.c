...
int32_t VL53L0X_write_multi(uint8_t address, uint8_t index, uint8_t  *pdata, int32_t count)
{
  ...
  res = i2c_writeBuffer(0, address, index, pdata, count);
...
//  res = GI2C1_ReadAddress(address, &index, sizeof(index), pdata, count);
  res = i2c_read(0, address, index, pdata, count);
  if (res!=EXIT_SUCCESS) {
    return 1; /* error */
  }
  return 0; /* ok */
...
int32_t VL53L0X_write_byte(uint8_t address,  uint8_t index, uint8_t data)
{
  ...
  res = i2c_write(0, address, index, data);
  if (res != EXIT_SUCCESS) {
    return 1; /* error */
  }
  return 0; /* ok */
...
int32_t VL53L0X_read_byte(uint8_t address,  uint8_t index, uint8_t  *pdata)
{
  ...
  res = i2c_readByte(0, address, index, pdata);
  if (res!=EXIT_SUCCESS) {
    return 1; /* error */
  }
  return 0; /* ok */
...
int32_t VL53L0X_read_word(uint8_t address,  uint8_t index, uint16_t *pdata)
{
  ...
  res = i2c_read(0, address, index, pdata, 2);
...
int32_t VL53L0X_read_dword(uint8_t address, uint8_t index, uint32_t *pdata)
{
  ...
  res = i2c_read(0, address, index, pdata, 4);
  if (res!=EXIT_SUCCESS) {
    return 1; /* error */
  }
  return 0; /* ok */
