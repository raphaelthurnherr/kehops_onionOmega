...
int pca9629_init(device_pca9629 pca9629config){
    ...
    err+= i2c_write(0, pca9629config.deviceAddress, 0x00, 0x20);    // MODE - Configuration du registre MODE (pin INT désactivée, Allcall Adr. désactivé)
    err+= i2c_write(0, pca9629config.deviceAddress, 0x01, 0xFF);    // WDTOI
    err+= i2c_write(0, pca9629config.deviceAddress, 0x02, 0x00);    // WDCNTL
    err+= i2c_write(0, pca9629config.deviceAddress, 0x03, 0x0F);    // IO_CFG
    err+= i2c_write(0, pca9629config.deviceAddress, 0x04, 0x10);    // INTMODE
    err+= i2c_write(0, pca9629config.deviceAddress, 0x05, 0x1F);    // MSK
    err+= i2c_write(0, pca9629config.deviceAddress, 0x06, 0x00);    // INTSTAT
    //err+= i2c_write(0, PCA9629, 0x07, 0x);    // IP
    err+= i2c_write(0, pca9629config.deviceAddress, 0x08, 0x00);    // INT_MTR_ACT
    err+= i2c_write(0, pca9629config.deviceAddress, 0x09, 0x00);    // EXTRASTEPS0
    err+= i2c_write(0, pca9629config.deviceAddress, 0x0A, 0x00);    // EXTRASTEPS1
    err+= i2c_write(0, pca9629config.deviceAddress, 0x0B, 0x10);    // OP_CFG_PHS
    err+= i2c_write(0, pca9629config.deviceAddress, 0x0C, 0x00);    // OP_STAT_TO
    err+= i2c_write(0, pca9629config.deviceAddress, 0x0D, 0x00);    // RUCNTL
    err+= i2c_write(0, pca9629config.deviceAddress, 0x0E, 0x00);    // RDCNTL
    err+= i2c_write(0, pca9629config.deviceAddress, 0x0F, 0x01);    // PMA - 0x01 Action unique, 0x00 action continue
    err+= i2c_write(0, pca9629config.deviceAddress, 0x10, 0x05);    // LOOPDLY_CW - Pour un delais de 20ms d'inversion de sens
    err+= i2c_write(0, pca9629config.deviceAddress, 0x11, 0x05);    // LOOPDLY_CCW Pour un delais de 20ms d'inversion de sens
    err+= i2c_write(0, pca9629config.deviceAddress, 0x12, 0xFF);    // CWSCOUNTL - Nombre de pas CW
    err+= i2c_write(0, pca9629config.deviceAddress, 0x13, 0xFF);    // CWSCOUNTH
    err+= i2c_write(0, pca9629config.deviceAddress, 0x14, 0xFF);    // CCWSCOUNTL - Nombre de pas CCW
    err+= i2c_write(0, pca9629config.deviceAddress, 0x15, 0xFF);    // CCWSCOUNTH
    err+= i2c_write(0, pca9629config.deviceAddress, 0x16, 0x9A);    // CWPWL - Vitesse / Largeur d'impulsion pour CW
    err+= i2c_write(0, pca9629config.deviceAddress, 0x17, 0x02);    // CWPWH
    err+= i2c_write(0, pca9629config.deviceAddress, 0x18, 0x9A);    // CCWPWL - Vitesse / Largeur d'impulsion pour CCW
    err+= i2c_write(0, pca9629config.deviceAddress, 0x19, 0x02);    // CCWPWH
    err+= i2c_write(0, pca9629config.deviceAddress, 0x1A, 0x00);    // MCNTL - Registre contrôle moteur
    err+= i2c_write(0, pca9629config.deviceAddress, 0x1B, 0xE2);    // SUBA1
    err+= i2c_write(0, pca9629config.deviceAddress, 0x1C, 0xE4);    // SUBA2
    err+= i2c_write(0, pca9629config.deviceAddress, 0x1D, 0xE8);    // SUBA3
    err+= i2c_write(0, pca9629config.deviceAddress, 0x1E, 0xE0);    // ALLCALLA
